...
export default function SettingsTab() {
  ...
  if (loading) {
    return <div className="text-sm text-kw-muted">설정 로드 중…</div>;
  }

  return (
    <div className="space-y-6 max-w-md">
      <div>
        <h3 className="text-base font-semibold text-kw-text mb-1">추출 엔진</h3>
        <p className="text-sm text-kw-muted mb-4">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`flex-1 rounded-lg border-2 p-4 text-left transition-colors
              ${extractor === "ezdxf"
                ...
                : "border-kw-border hover:border-kw-border-s bg-kw-surface"}`}
          >
            <div className="font-semibold text-sm text-kw-text mb-1">ezdxf</div>
            <div className="text-xs text-kw-muted">
              Python 기반 DXF 파싱. 빠르고 안정적.
            </div>
            {extractor === "ezdxf" && (
              <div className="mt-2 text-xs text-kw-accent font-medium">현재 사용 중</div>
            )}
          </button>
          ...
          >
            ...
            {extractor === "vision" && (
              <div className="mt-2 text-xs text-kw-warn font-medium">
                ⚠ 분석 실행 시 오류가 발생합니다.
              </div>
            )}
          </button>
        ...
        {error && <p className="text-xs text-kw-err mt-2">{error}</p>}
        {saving && <p className="text-xs text-kw-muted mt-2">저장 중…</p>}
      </div>
    </div>
  );
}
